...
export const aiService = {
   ...
   */
  async detectDisease(base64Image: string, mimeType: string, cropType: string): Promise<IDiseaseResponse> {
    const provider = getAIProvider();
    console.log(`[AI Disease Scan] Active Provider: ${provider}, cropType: "${cropType}"`);

    if (provider === 'openai') {
      console.log(`[Backend Request] Provider: openai, Model: gpt-4o-mini, Crop: "${cropType}"`);
      const client = getOpenAIClient();
      const promptText = `You are an expert plant pathologist. Analyze this crop leaf image of crop type: "${cropType}".
Return JSON object matching exactly this schema: {"diseaseName": "Disease Name", "confidence": 0.92, "treatment": "Step-by-step treatment."}`;
    ...
    if (provider === 'gemini') {
      ...
      console.log(`[Backend Request] Provider: gemini, Model: ${GEMINI_MODEL}, API Key: ${maskKey(apiKey)}, Crop: "${cropType}"`);
      const promptText = `You are an expert plant pathologist. Analyze this crop leaf image of crop type: "${cropType}".
Return a JSON object matching EXACTLY this schema (no extra fields, no markdown):
{
  ...
  "fertilizer": "string — specific fertilizing advice to strengthen plant defenses against the disease"
    ...
    // --- Mock Fallback Mode ---
    console.log(`[AI Disease Scan] Running mock fallback logic...`);
  ...
  async getSoilRecommendations(params: {
    ...
    const provider = getAIProvider();
    console.log(`[AI Soil Recs] Provider: ${provider}, crop: "${params.cropType}", pH: ${params.pH}, moisture: ${params.moisture}%`);
...
soilHealth must be a number from 1 to 10. riskLevel must be "Low", "Moderate", or "High".`;
    ...
    // --- Mock Fallback Mode ---
    console.log(`[AI Soil Recs] Running mock fallback logic...`);
  ...
  async predictYield(params: {
    ...
    console.log(`[AI Yield Predict] Active Provider: ${provider}, API Key: ${apiKeyPreview}, Model: ${GEMINI_MODEL}`);
...
Return ONLY a valid JSON object (no markdown) matching this exact schema:
{
  ...
  "recommendations": {
    ...
    // --- Mock Fallback Mode ---
    console.log(`[AI Yield Predict] Running mock fallback logic...`);
    const { cropType, area, fertilizer, season } = params;
    const yieldMap: Record<string, number> = { Wheat: 2.8, Corn: 4.5, Rice: 3.8, Tomato: 22.0, Soybean: 1.6, Cotton: 1.1, Sugarcane: 36.5, Potato: 12.0, Apple: 18.0 };
    const baseYield = area * (yieldMap[cropType] || 2.5) * (season === 'Winter' ? 0.4 : 1.0) * (fertilizer === 'None' ? 0.7 : 1.0);
    const priceMap: Record<string, number> = { Wheat: 280, Rice: 450, Tomato: 600, Cotton: 800, Sugarcane: 120 };
