...
router.post('/disease-detection', requireDB, requireAuth, [
  ...
  const { base64Image, mimeType, cropType, farmId } = req.body;
  const userId = req.user.id;
  ...
  try {
    ...
    const predictionData = await aiService.detectDisease(base64Image, mimeType, cropType);
...
router.post('/soil-analysis', requireDB, requireAuth, [
  ...
  try {
    const { farmId, moisture, pH, nitrogen, phosphorus, potassium, organicCarbon, temperature, humidity } = req.body;
    const userId = req.user.id;
...
router.post('/yield-predictions/predict', requireDB, requireAuth, [
  ...
  try {
    const { farmId, cropType, area, season, soilType, irrigation, fertilizer, historicalYield } = req.body;
    const userId = req.user.id;
