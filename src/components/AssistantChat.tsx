...
const getSuggestedPrompts = (lang: 'en' | 'hi') => {
  if (lang === 'hi') {
    return [
      { icon: AlertTriangle, text: "मेरे टमाटर के पौधों पर पीली पत्तियों का निदान करें" },
      { icon: Cloud, text: "अगले सप्ताह की बारिश मेरी फसल को कैसे प्रभावित करेगी?" },
      { icon: TrendingUp, text: "50 एकड़ मक्के की उपज का अनुमान लगाएं" },
      { icon: Sprout, text: "चिकनी मिट्टी के लिए जैविक उर्वरक की सिफारिश करें" }
    ];
  }
  return [
    { icon: AlertTriangle, text: "Diagnose yellowing leaves on my tomato plants" },
...
export default function AssistantChat({ user, language = 'en' }: AssistantChatProps) {
  ...
  useEffect(() => {
    ...
    const initialChat: ChatSession = {
      ...
      messages: [{
        ...
        role: 'assistant',
        content: language === 'hi'
          ? `नमस्ते ${user.name}, मैं आपका एंटरप्राइज एआई कृषि विज्ञानी हूँ। आज मैं आपकी खेती के कार्यों में कैसे सहायता कर सकता हूँ?`
          : `Hello ${user.name}, I am your Enterprise AI Agronomist. How can I assist you with your farming operations today?`,
  ...
  const createNewChat = () => {
    const newChat: ChatSession = {
      ...
      messages: [{
        ...
        role: 'assistant',
        content: language === 'hi'
          ? `नमस्ते ${user.name}, आज मैं आपकी कैसे सहायता कर सकता हूँ?`
  ...
  const handleSend = async (content: string) => {
    ...
    try {
      const payload: any = { userId: user.id, message: trimmed, language };
      if (imageToSend) {
        ...
        {sidebarOpen && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-4 no-scrollbar">
              ...
              <div>
                <h4 className="px-3 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{language === 'hi' ? 'हालिया बातचीत' : 'Recent'}</h4>
            ...
            <div className="p-4 border-t border-white/10 text-center">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{language === 'hi' ? 'लाइव मॉडल कनेक्टेड' : 'Live Model Connected'}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-transparent relative z-10">
        ...
        <div className="h-16 px-4 border-b border-white/10 flex items-center justify-between bg-white/5 backdrop-blur-md">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <h2 className="text-base font-bold text-white truncate max-w-[200px] md:max-w-md">{activeChat?.title || (language === 'hi' ? 'कोई बातचीत नहीं चुनी गई' : 'No Conversation Selected')}</h2>
          </div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="py-10">
                  <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl font-black text-white mb-2">
                      {language === 'hi' ? 'एंटरप्राइज एआई कृषि विज्ञानी' : 'Enterprise AI Agronomist'}
                    </h1>
                    <p className="text-[#E9D5FF] max-w-md mx-auto">
                      {language === 'hi' 
                        ? 'फसल प्रबंधन, रोग निदान और उपज अनुकूलन के लिए आपका समर्पित बुद्धिमान सहायक।' 
                        : 'Your dedicated intelligent assistant for crop management, disease diagnostics, and yield optimization.'}
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    {getSuggestedPrompts(language).map((prompt, i) => (
                      <button 
                        key={i}
                  ...
                  <div className="flex flex-wrap justify-center gap-3">
                    ...
                    >
                      <ImageIcon className="h-3.5 w-3.5" /> {language === 'hi' ? 'छवि संलग्न करें' : 'Attach Image'}
                    </button>
                    <button 
                    ...
                    >
                      <Mic className="h-3.5 w-3.5" /> {language === 'hi' ? 'आवाज इनपुट' : 'Voice Input'}
                    </button>
                    <button className="px-4 py-2 bg-black/40 border border-white/5 hover:border-white/20 rounded-xl text-xs font-bold text-gray-400 hover:text-white flex items-center gap-2 transition-colors">
                      <Activity className="h-3.5 w-3.5" /> {language === 'hi' ? 'रोग स्कैन' : 'Disease Scan'}
                    </button>
                    <button className="px-4 py-2 bg-black/40 border border-white/5 hover:border-white/20 rounded-xl text-xs font-bold text-gray-400 hover:text-white flex items-center gap-2 transition-colors">
                      <Map className="h-3.5 w-3.5" /> {language === 'hi' ? 'फार्म डेटा' : 'Farm Data'}
                    </button>
                  </div>
                </motion.div>
              )}
          ...
          <div className="max-w-4xl mx-auto relative">
            ...
            <div className="bg-black/60 border border-white/20 rounded-3xl p-2 pr-4 flex items-end gap-2 shadow-2xl backdrop-blur-xl focus-within:border-[#9333EA] transition-colors">
              ...
              <textarea
                ...
                placeholder={language === 'hi' ? "एआई कृषि विज्ञानी को संदेश भेजें... (नई लाइन के लिए Shift+Enter)" : "Message AI Agronomist... (Shift+Enter for new line)"}
              ...
              <div className="flex items-center gap-2 shrink-0 pb-1">
                ...
                <button 
                  ...
                  title={
                    micStatus === 'unsupported' ? (language === 'hi' ? 'भाषण पहचान समर्थित नहीं है' : 'Speech recognition not supported') :
                    micStatus === 'listening' ? (language === 'hi' ? 'रिकॉर्डिंग रोकने के लिए क्लिक करें' : 'Click to stop recording') :
                    micStatus === 'processing' ? (language === 'hi' ? 'भाषण को प्रोसेस किया जा रहा है...' : 'Processing speech...') :
                    (language === 'hi' ? 'आवाज इनपुट शुरू करने के लिए क्लिक करें' : 'Click to start voice input')
                  }
                >
                  {micStatus === 'listening' ? (
                ...
                <button
                  ...
                  title={isGenerating ? (language === 'hi' ? 'एआई जनरेट कर रहा है...' : 'AI is generating...') : (language === 'hi' ? 'संदेश भेजें' : 'Send message')}
                >
                  {isGenerating ? (
              ...
              <div className="flex items-center gap-3">
                ...
                {micStatus === 'processing' && (
                  <span className="text-[10px] text-yellow-400 font-bold uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    {language === 'hi' ? 'प्रोसेस कर रहा है...' : 'Processing...'}
                  </span>
                )}
                {attachedImage && (
                  <span className="text-[10px] text-[#D946EF] font-bold uppercase tracking-wider flex items-center gap-1.5">
                    <ImageIcon className="h-3 w-3" />
                    {language === 'hi' ? '1 छवि संलग्न है' : '1 image attached'}
                  </span>
                )}
              </div>
              <span className="text-[10px] text-gray-500 font-medium">{language === 'hi' ? 'एंटरप्राइज एआई कृषि विज्ञानी गलतियाँ कर सकता है। महत्वपूर्ण कृषि निर्णयों को सत्यापित करें।' : 'Enterprise AI Agronomist can make mistakes. Verify critical farming decisions.'}</span>
            </div>
          </div>
        </div>
      </div>
